...
| What a place is | Two sentences from the English Wikipedia article, saved once for the catalogue and credited in small type, or a general note for that kind of place when there is no article | The same, looked up live for any place found on Google Maps, and shown only when the article's title holds every word of the place name | `src/lib/wikipedia.ts`, `src/features/places/useWikipediaSummary.ts` |

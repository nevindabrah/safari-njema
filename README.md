...
| What a place is | Two sentences from Wikipedia, or from the Wikivoyage entry for the place when Wikipedia has no article, saved once for the catalogue and credited in small type, or a general note for that kind of place when neither has it | The same, looked up live for any place found on Google Maps: Wikipedia only when the article's title holds every word of the place name, Wikivoyage matched by name or by being within 150 metres, plus Wikipedia's lead photo when it is freely licensed on Wikimedia Commons | `src/lib/wikipedia.ts`, `src/features/places/useWikipediaSummary.ts` |

...
- The phrasebook has a short section on telling time in Swahili: why 7 am is hour one, the add or take away six rule, and a slider that shows any hour both ways (`src/features/time/`, `src/lib/swahiliTime.ts`). Its Swahili words were not part of the teacher's review, so the section carries a "Not yet reviewed" label.

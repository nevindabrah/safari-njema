...
- The phrasebook has a short section on telling time in Swahili: why 7 am is hour one, the add or take away six rule, and an interactive clock (`src/features/time/`, `src/lib/swahiliTime.ts`). The clock has two rings of numbers, the watch hour outside and the Swahili hour inside, so the hand points at both at once. Tap an hour or drag the hand, switch am and pm, and the face turns dark at night. A practice mode names a time in Swahili and asks the learner to set the clock. Its Swahili words were not part of the teacher's review, so the section carries a "Not yet reviewed" label.

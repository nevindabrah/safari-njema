...
ok('accepting the friendship already put D on C\'s trip, and an explicit invite is harmless', await as(c, async () => { const before = await count(`select count(*) n from trip_members where trip_id = '${tripC}' and user_id = '${d}'`); await db.query(`insert into trip_members (trip_id, user_id, added_by) values ('${tripC}', '${d}', '${c}') on conflict do nothing`); return before === 1 }))
...
const f1 = (await db.query(`insert into auth.users (email, raw_user_meta_data) values ('fatma@example.com', '{"username":"fatma_f"}') returning id`)).rows[0].id
const f2 = (await db.query(`insert into auth.users (email, raw_user_meta_data) values ('george@example.com', '{"username":"george_g"}') returning id`)).rows[0].id
const tripF1 = await as(f1, async () => (await db.query(`insert into trips (user_id, title) values ('${f1}', 'F1 trip') returning id`)).rows[0].id)
await as(f1, async () => { await db.query(`insert into friendships (requester_id, addressee_id) values ('${f1}', '${f2}')`) })
ok('a pending request shares nothing yet', await as(f2, async () => (await count(`select count(*) n from trips where id = '${tripF1}'`)) === 0))
await as(f2, async () => { await db.query(`update friendships set status = 'accepted' where addressee_id = '${f2}'`) })
ok('accepting a request puts the friend on the other\'s trip', await as(f2, async () => (await count(`select count(*) n from trips where id = '${tripF1}'`)) === 1))
const tripF2 = await as(f2, async () => (await db.query(`insert into trips (user_id, title) values ('${f2}', 'F2 trip') returning id`)).rows[0].id)
ok('a trip made after the friendship includes the friend', await as(f1, async () => (await count(`select count(*) n from trips where id = '${tripF2}'`)) === 1))
ok('a friend can add a stop to the shared trip and each keeps their own lesson', await (async () => { const stop = await as(f2, async () => (await db.query(`select add_trip_stop('${tripF1}', 'g-auto', 'Karura Forest', -1.24, 36.83, '{}', 'park', 'nairobi', null, '{}') id`)).rows[0].id); await as(f1, async () => { await db.query(`insert into user_lessons (user_id, trip_stop_id, content) values ('${f1}', '${stop}', '{}'::jsonb)`) }); return as(f2, async () => (await count(`select count(*) n from user_lessons where trip_stop_id = '${stop}'`)) === 0) })())
const justMe = await as(f2, async () => (await db.query(`select add_trip_stop('${tripF1}', 'g-mine', 'Nairobi Hospital', -1.29, 36.8, '{}', 'other', 'nairobi', null, '{}', true) id`)).rows[0].id)
ok('a "Just me" stop is visible to the person who added it', !!justMe && (await as(f2, async () => (await count(`select count(*) n from trip_stops where id = '${justMe}'`)) === 1)))
ok('the trip owner does not see a friend\'s "Just me" stop', await as(f1, async () => (await count(`select count(*) n from trip_stops where id = '${justMe}'`)) === 0))
ok('the trip owner cannot change a "Just me" stop they cannot see', await as(f1, async () => (await db.query(`update trip_stops set visit_date = '2026-10-10' where id = '${justMe}'`)).affectedRows === 0))
const ownersStop = await as(f1, async () => (await db.query(`select add_trip_stop('${tripF1}', 'g-theirs', 'Uhuru Park', -1.29, 36.82, '{}', 'park', 'nairobi', null, '{}') id`)).rows[0].id)
ok('a member cannot make someone else\'s stop private', !!ownersStop && await as(f2, async () => { try { await db.query(`update trip_stops set private = true where trip_id = '${tripF1}' and user_id = '${f1}'`); return (await count(`select count(*) n from trip_stops where trip_id = '${tripF1}' and private and user_id = '${f1}'`)) === 0 } catch { return true } }))
ok('a stop added without the flag is shared as before', await as(f1, async () => (await count(`select count(*) n from trip_stops where trip_id = '${tripF1}' and not private`)) >= 1))
await as(f1, async () => { await db.query(`delete from friendships where requester_id = '${f1}' and addressee_id = '${f2}'`) })
ok('ending the friendship removes both from each other\'s trips', (await as(f2, async () => (await count(`select count(*) n from trips where id = '${tripF1}'`)) === 0)) && (await as(f1, async () => (await count(`select count(*) n from trips where id = '${tripF2}'`)) === 0)))

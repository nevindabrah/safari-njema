// Prints, as one JSON list, the Swahili for every time the clock can show (288 of them) plus "Saa ngapi?".
...
for (let hour = 0; hour < 24; hour++) {
  for (let minute = 0; minute < 60; minute += 5) phrases.add(sayTime(swahiliTime(hour, minute)))
}

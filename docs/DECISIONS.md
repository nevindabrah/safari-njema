...
### 5.5a The template brief names the place
**Decision.** The template's first sentence is built from the stop: "Java House is a restaurant in Nairobi." Then the kind's paragraph follows. When the brief step also shows a Wikipedia paragraph, that first sentence is dropped so the place is not introduced twice. The lesson header note reads "Phrases chosen for Java House from the reviewed phrase bank" instead of "General lesson for this kind of place".
**Why.** The owner wanted lessons that read as being about the stop the student is going to. Without a model, the place name, kind and region are what the app knows for certain, so the brief says exactly that and no more.
**Alternatives.** Leaving the brief general (read as a canned lesson). Letting a model write it (unreviewed text, and a key).

### 5.5b Google's detailed type decides the lesson, not just the broad kind
**Decision.** Every stop already stores the full list of types Google gives it, such as `coffee_shop`, `ferry_terminal` or `shopping_mall`, and until now the app collapsed that list to one of eleven kinds and threw the rest away. A flavour table in `placeFlavour.ts` reads the most specific type and supplies four things: the noun the brief opens with ("a coffee house" rather than "a restaurant"), one or two practical notes, extra phrase tags and extra phrase slots. A flavour may also replace the broad kind's paragraph and notes outright, and may name tags to keep out. A shopping centre therefore says prices are printed and fixed, teaches numbers and paying, and never teaches bargaining, while a market still teaches bargaining. On the coast, a coastal greeting is added at the end of the plan, unless the place type already asks for one, so a mosque still opens with the respectful greeting.
Four kinds were added beside the original eleven, for the everyday places a student actually needs: hospital or clinic, school or university, bank, and office or service. Each has its own icon, label, brief and phrase set, so a hospital teaches how to say you are unwell and a bank teaches numbers. `places.place_type` is a plain text column with no check constraint, so this needed no migration.
**Why.** Lessons for places outside the 51 built-in ones were the weakest part of the app: every restaurant in Kenya got the same lesson, and a hospital or a school was simply "Place". Google's types are free, already stored, and far more precise than our eleven kinds, so this cost nothing and needed no key.
**Alternatives.** Claude writing a brief per place (a key, a bill, and unreviewed text). Widening the eleven kinds to forty (every screen, icon and phrase table would have to grow with them). Reading the Wikipedia text for keywords (guessy, and it would put a wrong note under a right description).
**Trade-off.** The flavour notes are English guidance written for this app, not reviewed Swahili, and they assert facts about Kenya that a local reader should check. No flavour writes Swahili, and a test enforces that.

### 5.5c An uncertain place teaches the phrases that work anywhere
**Decision.** When Google's types match none of the eleven kinds, the stop falls to the "other" kind, and that lesson is now the universal set: the greeting, the reply, asking a name, asking for help, numbers, left and right, and goodbye. Greetings are scored up for an uncertain place even when it is not the first stop, which is the opposite of the rule everywhere else. The brief says plainly that these are the words that work anywhere in Kenya.
**Why.** Guessing wrongly about a place is worse than admitting we do not know it. If the app cannot tell what somewhere is, the honest lesson is the survival Swahili a student needs at every stop.
**Alternatives.** Refusing to build a lesson (a stop with no lesson looks broken). Falling back to the city lesson (directions and transport are not always what an unknown place needs).

### 5.6 What a place is, and what it looks like, comes from Wikipedia and Wikivoyage, not from a model or a paid photo
**Decision.** The brief step and the add card open with two sentences about the place from the English Wikipedia article, credited in very small type with a link to the article and to CC BY-SA 4.0. The 51 catalogue places have their descriptions saved in `placeSummaries.json` by a script run once and read by a person (43 of 51 have a fitting article; the rest show nothing). A place found on Google Maps is looked up live: first the article with exactly that title, then a quoted search steered by the county. An article is used only if every word of the place name appears in its title, and disambiguation pages and stubs are dropped. When Wikipedia has no article, the app reads the Wikivoyage page for the stop's town (chosen from its county, or from its region for a saved stop) and looks for the entry whose name matches, or failing that the entry within 150 metres of the stop's coordinates. Wikivoyage lists the restaurants, hotels, bars and sights that Wikipedia does not: about 100 described entries each for Nairobi and Mombasa, 60 for Diani. Its text is trimmed to two sentences and credited as Wikivoyage under the same licence. When neither source has it, the add card shows a general note for that kind of place instead, marked as general in the same small type, and the lesson brief shows only its own template paragraph, which is general already. A wrong article is never shown. A place found on Google Maps also gets the article's lead image when that image is on Wikimedia Commons under a free licence (Creative Commons, CC0 or public domain), is a photo rather than a map or drawing, and is at least 600 pixels wide. It is served from Wikimedia's own thumbnail service at 500, 960 and 1920 pixels, the only widths Wikimedia serves to outside sites, with the author and licence shown in the same small credit as the catalogue photos. Places with no free image keep the icon tile. Everything found is remembered in the browser so a stop is looked up once, not on every visit.
...
**Alternatives.** Google's editorial summaries (good for businesses, but in the dear price band and only cacheable for 30 days). The place's own website (browsers cannot read other sites, and Talisman's refused a server request outright). Wikidata (a few words at best). OpenStreetMap (facts like cuisine and hours, no prose). Google's place photos (billed per view and not allowed to be stored). Wikimedia Commons photos taken near the coordinates (tried; the files nearest Maasai Market were an exhibition poster and a meditation graphic, so a nearby photo is not a photo of the place). Claude writing the brief (costs money and writes text nobody reviewed). A free tier model key (still a key to keep secret and a quota to run out). Writing 51 briefs by hand (the owner's time, and nothing for places outside the catalogue). Showing whatever Wikipedia's search returns first (the first run matched Giraffe Centre to Giraffe Manor and Talisman Restaurant to a September 11 article).
A place with no free photo of its own falls back to a photo of its town, taken from the town's own Wikipedia article and labelled "Illustrative photo of Nairobi" in the credit, so nobody reads it as a photo of the restaurant. That stand-in appears only on the large surfaces, the add card, the lesson header and the pocket card, where one is seen at a time. The small thumbnails in the itinerary keep the patterned tile, because three stops in one town would otherwise show the same skyline three times down the list. Town photos are chosen from the same town list the descriptions use, filenames that look like maps, flags, seals or diagrams are refused, and the image must be landscape. When even that fails the tile is a kanga style print drawn in CSS, a lattice of small rings and dots inside a border band with the place's icon in the middle, finer on the small thumbnails than on the large cards. The middle is masked out so the print never runs behind the icon. It is a design rather than a gap, and it needs no image file.
...
### 6.8 The map's own zoom and recentre buttons
**Decision.** Google's default controls stay switched off and the map carries three of our own buttons on the right edge: zoom in, zoom out, and recentre. Recentre fits the box around every stop, centres on the single stop when there is only one, and falls back to the whole country when the trip is empty. The maths is a pure function in `src/lib/mapFrame.ts` with tests, so the map component only calls the map. The buttons sit vertically centred on the right because the search box owns the top of the map and the place card owns the bottom.
**Why.** Pinch to zoom is awkward one handed on a phone, and after following a pin there was no way back to the whole trip.
**Alternatives.** Google's own controls (they do not match the app's shapes or colours, and they cannot be moved out of the corners the app already uses). A bottom right stack in the usual place (the place card covers that corner on a laptop).
**Trade-off.** The sketch map used when there is no Maps key does not zoom, so it has no buttons. The browser tests run against that sketch map, so these buttons are covered by the unit tests on the framing maths rather than by a browser test.

### 6.7 With a Maps key, the catalogue and Google Maps search together
**Decision.** One search box whichever way the site is configured. Without a key it searches the catalogue. With a key the same box lists catalogue matches first, with their photos, then places from Google Maps under them, and the catalogue sheet does the same below its grid. The Google Maps script is loaded once around the whole trip screen by `MapsProvider`, so the map, the floating search and the catalogue share it. Nothing Google is rendered without a key, so the demo and the browser tests never load the script.
**Why.** The catalogue's intent search ("food", "safari") and reviewed photos are worth keeping when the real map arrives, and a place outside the catalogue must still be findable. Two search boxes would have been two things to explain.
**Alternatives.** Google only when the key exists (loses intent search and photos). Google's own autocomplete widget (does not match the design). Searching Google from the catalogue only (the map's search box is where people type first).
Tapping a numbered pin shows the stop's name in a pill above it, as the sketch map does. Google's `tourist_attraction` type, which many landmarks carry with nothing more specific, maps to the museum or landmark kind so those stops get a real lesson instead of the general one; a more specific type still wins.
**Trade-off.** Each typed search past two letters costs one autocomplete request when a key exists; session tokens make a search plus a pick bill as one session. This path has been type checked against Google's current types and read against the current documentation but has not run against a live key yet, because the owner has not created one.

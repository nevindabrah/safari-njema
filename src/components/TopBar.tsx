...
// Exists so every screen has the same way to get around. On a phone the links move to a menu button and to BottomTabs. Between a phone and a laptop every link still shows, made to fit by tighter padding, a shorter word for the phrasebook and the mark alone in place of the full name.
...
const link = ({ isActive }: { isActive: boolean }) => `px-2 lg:px-3 py-3 rounded-pill ${isActive ? 'bg-tint' : 'hover:bg-tint'}`

export function TopBar() {
  const { user } = useAuth()
  const badges = useBadges()
  const { profile } = useProfile()
  const wide = 'hidden min-[700px]:inline-block'
        ...
        <div className="mx-auto max-w-6xl px-4 h-16 flex items-center justify-between gap-2">
          ...
          <nav className="flex items-center gap-1 text-sm font-bold whitespace-nowrap" aria-label="Main">
            ...
            {user ? (
              ...
              <Link to="/login" className="hidden min-[700px]:inline-flex px-4 min-h-[44px] items-center rounded-pill bg-primary text-on-primary">Log in</Link>
            )}
          </nav>
        </div>
        <DemoBanner />

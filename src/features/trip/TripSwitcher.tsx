...
export function TripSwitcher({ shared }: { shared: SharedTrip[] }) {
  ...
  const pill = ({ isActive }: { isActive: boolean }) => `inline-flex items-center min-h-[40px] max-w-[16rem] px-4 rounded-pill text-sm font-bold whitespace-nowrap truncate ${isActive ? 'bg-primary text-on-primary' : 'bg-soft text-on-soft'}`
  return (

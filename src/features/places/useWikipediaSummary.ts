...
// Exists so lessons and the add card can say what a place is without an AI key. A wrong or missing article means nothing is shown.
...
export function useWikipediaSummary(googlePlaceId: string | null, name: string, county: string | null = null): WikipediaSummary | null {
  const saved = googlePlaceId ? summaryFor(googlePlaceId) : null
  const builtIn = Boolean(googlePlaceId?.startsWith('sample-'))
  const [found, setFound] = useState<WikipediaSummary | null>(() => remembered.get(name) ?? null)

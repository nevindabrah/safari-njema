      ...
      <div className="flex items-start gap-3">
        {phrase.clip ? (
          <button type="button" onClick={() => playClip(phrase.clip!)} aria-label={`Listen to ${phrase.swahili}`} title="Listen"
            className="shrink-0 w-11 h-11 rounded-pill flex items-center justify-center cursor-pointer active:translate-y-[2px] transition-transform"
            style={phrase.held ? { background: 'var(--wrong-soft)', color: 'var(--text)' } : { background: 'var(--hero)', color: 'var(--on-hero)' }}>
